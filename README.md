    ...
    <script>
        ...
        function loadDataAndInitialize() {
            auth.onAuthStateChanged(user => {
                if (user) {
                    currentUser = user;
                    // For simplicity, we'll use a custom claim to identify the admin.
                    // You'll need to set this custom claim on the admin user's account.
        ...
        function loginEmployee() {
            ...
            auth.signInWithEmailAndPassword(email, password)
                .catch(error => {
                    alert("E-mail ou mot de passe incorrect. Veuillez réessayer. " + error.message);
        ...
        function saveUserHours() {
            if (!currentUser || localStorage.getItem(ADMIN_STATE_KEY) === 'true') return;
         ...
         */
        function promptForNewPassword(employeeUid, employeeEmail) {
            if (localStorage.getItem(ADMIN_STATE_KEY) !== 'true') return;
            const newPassword = prompt(`Définir un nouveau mot de passe pour l'employé "${employeeEmail}" :`);

            if (newPassword === null || newPassword.trim().length === 0) {
                if (newPassword !== null) alert("Le mot de passe ne peut pas être vide.");
                return;
            }
            // You would need a backend function to update the user's password.
            // This is not possible from the client-side for security reasons.
            alert("La mise à jour du mot de passe doit être implémentée côté serveur.");
        }
        ...
        function addEmployeeField() {
            if (localStorage.getItem(ADMIN_STATE_KEY) !== 'true') return;
            const email = prompt("Veuillez saisir l'E-MAIL du nouvel employé :").trim();
            if (email === null || email.length === 0) {
                if (email !== null) alert("L'e-mail de l'employé ne peut pas être vide. Ajout annulé.");
                return;
            }

            const password = prompt(`Veuillez définir le mot de passe initial pour le nouvel employé "${email}" :`);
            ...
            // You would need a backend function to create a new user.
            // This is not possible from the client-side if you want to control it from an admin account.
            alert("La création d'utilisateurs doit être implémentée côté serveur.");
        }

        function loadEmployees() {
            // This function would now fetch users from Firebase Authentication, not localStorage.
            // This requires a backend implementation to list users.
            const list = document.getElementById('employee-list');
            list.innerHTML = '<p>Le chargement des employés depuis Firebase Auth nécessite une implémentation backend.</p>';
            updateEmployeeControlsVisibility();
        }
        ...
        // ... (The rest of the functions for tips and history management remain largely the same,
        // but you might want to store this data in Firestore or Realtime Database instead of localStorage
